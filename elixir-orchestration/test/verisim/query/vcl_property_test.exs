...
defmodule VeriSim.Query.VCLPropertyTest do
  ...
  defp valid_proof_type do
    member_of(
      ~w(existence integrity consistency provenance freshness access citation custom zkp proven sanctify)
    )
  end
  ...
  defp valid_query_ast do
    gen all(
          modalities <- list_of(valid_modality(), min_length: 1, max_length: 4),
          proof_specs <- list_of(proof_spec_raw(), min_length: 1, max_length: 3)
        ) do
      %{
  ...
  property "multi-proof specs split correctly on AND/OR" do
    check all(
            types <- list_of(valid_proof_type(), min_length: 2, max_length: 4),
            contracts <- list_of(valid_contract_name(), length: length(types))
          ) do
