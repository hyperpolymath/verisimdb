...
defmodule VeriSim.Query.VCLTest do
  ...
  describe "VCLBridge.parse/1 — SELECT queries" do
    ...
    test "parses multi-modality query" do
      {:ok, ast} = VCLBridge.parse("SELECT GRAPH.*, VECTOR.*, DOCUMENT.* FROM HEXAD 'abc-123'")
    ...
    test "parses query with WHERE clause" do
      {:ok, ast} =
        VCLBridge.parse("SELECT DOCUMENT.* FROM HEXAD 'abc-123' WHERE DOCUMENT.title = 'Test'")

      assert is_map(ast)
    end

    test "parses query with LIMIT and OFFSET" do
      {:ok, ast} = VCLBridge.parse("SELECT GRAPH.* FROM HEXAD 'abc-123' LIMIT 10 OFFSET 5")
    ...
    test "parses query with ORDER BY" do
      {:ok, ast} =
        VCLBridge.parse("SELECT DOCUMENT.* FROM HEXAD 'abc-123' ORDER BY DOCUMENT.title ASC")

      assert is_map(ast)
    end

    test "parses query with aggregate in projection" do
      {:ok, ast} = VCLBridge.parse("SELECT GRAPH.* FROM HEXAD 'abc-123' LIMIT 5")
  ...
  describe "VCLBridge — mutations via parse_statement/1" do
    test "parses INSERT with document data" do
      {:ok, ast} =
        VCLBridge.parse_statement("INSERT HEXAD WITH DOCUMENT(title = 'Test', body = 'Content')")
    ...
    test "parses UPDATE with set clause" do
      {:ok, ast} =
        VCLBridge.parse_statement("UPDATE HEXAD 'abc-123' SET DOCUMENT.title = 'Updated'")
  ...
  describe "condition classification" do
    test "simple condition is pushed down" do
      {:ok, ast} =
        VCLBridge.parse("SELECT GRAPH.* FROM HEXAD 'abc-123' WHERE GRAPH.type = 'Person'")
