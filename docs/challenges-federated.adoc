...
**Problem:** Every registry update (Octad registration, policy change) requires **Raft quorum consensus**.
...
[source,text]
----
Operation: Query Octad across 3 federated stores
...
|**Data Tampering**
|Store A returns modified Octad content
...
1. **Compromise Surface:** Any one store compromise exposes its hosted Octads
...
Attacker compromises University B's federated store:
  - Gains access to all Octads hosted by University B
  - Can serve malicious data to orchestrator
  - Can exfiltrate query patterns (who queries what)

Impact: Partial breach (only University B's Octads), but:
...
[source,text]
----
1. Orchestrator queries all stores hosting Octad 550e8400-...
   - Store A (Graph): GET /octad/550e8400/graph/metadata
   - Store B (Vector): GET /octad/550e8400/vector/metadata
   - Store C (Document): GET /octad/550e8400/document/metadata
...
[source,text]
----
T=0:   Client writes Octad update to orchestrator
...
[source,text]
----
Proposal: Add "license" field to Octad metadata
...
[source,text]
----
Scenario: EU user queries Octad hosted across:
...
**Consequences:**
- **Compliance Overhead:** Must track **data residency** for every Octad
...
**Mitigation:**
- **Data Tagging:** Tag Octads with **jurisdiction metadata** (e.g., "EU-only", "US-only")
...
[source,text]
----
User requests deletion of Octad 550e8400-...

VeriSimDB must:
  1. Identify all stores hosting any modality of Octad
  ...
  5. Update registry to mark Octad deleted
...
- **Immutable Flag:** Mark Octad as "deleted" in registry immediately, lazy-delete from stores
- **Cryptographic Deletion:** Encrypt Octads with **per-Octad keys**, delete keys instead of data
...
[source,text]
----
Scenario: French research paper (Octad X) hosted only on French stores
