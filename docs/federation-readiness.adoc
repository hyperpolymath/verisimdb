...
* **No cross-modal consistency checks across instances.** VQL's `PROOF CONSISTENCY` clause can request verification that graph, vector, and semantic modalities agree across federation peers. Without VCL-DT, there is no way to express or verify cross-instance consistency at query time.
...
* Wire Lean type checker into the VCL-DT execution path (prerequisite: VCL-DT Phase 3 from link:vcl-vs-vcl-dt.adoc[VQL Slipstream vs VCL-DT]).
