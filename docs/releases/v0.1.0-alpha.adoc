...
____
⚠️ *HISTORICAL RELEASE NOTE (2026-07-07)* — the "`Production-Ready`" and
"`100% Complete`" claims in this document were inaccurate and are
superseded by `+KNOWN-ISSUES.adoc+` (see issues #29/#30/#31). Retained
for release history only.
____
...
VeriSimDB v0.1.0-alpha is the first public release of the Veridical
Simulacrum Database - a multimodal database with self-normalization
capabilities. This release marks the completion of all core
functionality and readiness for production deployment.

VeriSimDB operates as both a standalone database (like PostgreSQL) and a
federated coordinator for distributed knowledge networks.
...
* *VQLError.res* - Comprehensive error types for all failure modes (14K+
lines)
* *VQLExplain.res* - Query execution plan visualization (7K+ lines)
* *VQLTypeChecker.res* - Dependent-type verification with ZKP
integration
...
Every octad (entity) exists simultaneously across six synchronized
representations:

[arabic]
. *Graph* (244 LOC) - RDF triples and property graphs via Oxigraph
. *Vector* (248+ LOC) - HNSW similarity search for embeddings
. *Tensor* (278 LOC) - Multi-dimensional numeric data with ndarray/Burn
. *Semantic* (345 LOC) - Type annotations and CBOR proof blobs
. *Document* (Complete) - Full-text search powered by Tantivy
. *Temporal* (377+ LOC) - Version history and time-travel queries

*Additional Components:* - *Octad Store* (400+ LOC) - Unified entity
management - *Drift Detection* (484+ LOC) - Cross-modal consistency
monitoring - *Normalizer* (406 LOC) - Self-normalization when drift
exceeds thresholds - *HTTP API* (782 LOC) - RESTful API server with Axum
...
*Rust Tests (12 tests):* - Octad CRUD operations - Cross-modal
consistency - Drift detection - Vector similarity search - Fulltext
search - Temporal versioning - Graph relationships - Normalization -
Multi-modal queries - Concurrent operations

*Elixir Tests (Full stack):* - RustClient integration - QueryRouter -
DriftMonitor - SchemaRegistry - EntityServer - VCLExecutor - End-to-end
integration

==== ⚡ Performance Benchmarks (100% Complete)

Criterion-based benchmarks for: - Document store: create, search (1K
docs) - Vector store: insert, similarity search across 128/384/768
dimensions (10K vectors) - Graph store: node/edge operations - Octad
operations: create, retrieve - Drift detection: calculation performance
...
Comprehensive 100+ section guide covering: - Three deployment modes:
Standalone, Federated, Hybrid - Hardware/software requirements -
Complete deployment steps with Podman - Security: TLS, authentication,
RBAC, encryption - Monitoring: Prometheus metrics, logging, alerting -
Backup & recovery procedures - Performance tuning - Troubleshooting -
Operational procedures - Production checklists
...
*Prerequisites:* - Rust 1.75+ - Elixir 1.16+ with Erlang/OTP 26+ -
...
* `+VERISIM_MODE+` - Deployment mode: standalone, federation, hybrid
(default: standalone)
* `+VERISIM_DATA_DIR+` - Data directory (default: /var/lib/verisimdb)
* `+VERISIM_LOG_LEVEL+` - Log level: debug, info, warn, error (default:
info)
...
_Note: Performance varies based on hardware, configuration, and data
size_
...
[arabic]
. *RUSTSEC-2026-0002* - lru 0.12.5 (transitive dependency, LOW severity)
...
Third-party components retain their original licenses (MIT, Apache, BSD,
etc.).
...
*Development:* - Jonathan D.A. Jewell (j.d.a.jewell@open.ac.uk)

*AI Assistance:* - Claude Sonnet 4.5 (Anthropic)

*Open Source Libraries:* - Oxigraph (RDF/SPARQL) - Tantivy (Full-text
search) - Axum (HTTP server) - Elixir/OTP (Orchestration) - And many
others (see dependencies)
...
[arabic]
. *Deploy:* Follow `+DEPLOYMENT.adoc+` for your environment
. *Test:* Run integration tests and benchmarks
. *Experiment:* Create octads and run VCL queries
. *Feedback:* Report issues and contribute
...
_Released with ❤️ by the hyperpolymath project_
