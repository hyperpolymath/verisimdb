...
4. **VCL-DT** -- a dependent-type extension to the VeriSim Consonance Language (VCL) that supports 11 proof types, enabling clients to request and verify formal certificates of data consistency, integrity, and provenance.
...
VCL-DT (VQL with Dependent Types) is activated by the presence of a `PROOF` clause.
...
- **verisim-spatial**: Haversine brute-force geospatial queries (radius, bounding box, k-nearest); R-tree index planned.
...
- **VeriSim.QueryRouter**: Routes VQL queries to the appropriate execution pipeline (slipstream vs VCL-DT) and handles federation decomposition.
