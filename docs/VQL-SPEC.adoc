...
// ============================================================================
// APPENDIX E: VQL-dt++ EXTENSIONS (TYPELL INTEGRATION)
// ============================================================================

[appendix]
== VQL-dt++ Extensions (Typell Integration)

VQL-dt++ extends VQL v3.0 with six optional clauses that provide maximal type-theoretic strictness. These extensions are consumed by the **Typell verification kernel** (the formal verification substrate for PanLL) and are specified normatively in the grammar delta file:

* link:../../typeql-experimental/docs/vql-dtpp-grammar.ebnf[`vql-dtpp-grammar.ebnf`] — EBNF grammar delta (199 lines)

All six clauses are optional and composable in any combination. They append after the standard query structure with no keyword conflicts against VQL v3.0's 60+ reserved keywords.

=== VQL-dt vs VQL-dt++ Feature Comparison

[cols="2,1,1,2"]
|===
| Feature | VQL-dt | VQL-dt++ | Typell Kernel Mapping
...
VQL-dt++ queries are verified by the Typell kernel via JSON-RPC:

[source]
----
typell.check(query, "vql-dt++") → TypeResult
----
...
`[.planned]` All six VQL-dt++ clauses. Grammar formally specified. Implementation pending Typell kernel (Phase 3+).
