...
/// `INSERT '<title>' '<body>'`
async fn execute_insert(state: &AppState, raw: &str) -> Result<VclExecuteResponse, ApiError> {
...
async fn execute_show(state: &AppState, tokens: &[String]) -> Result<VclExecuteResponse, ApiError> {
    if tokens.len() < 2 {
        return Err(ApiError::BadRequest(
