...
type TestOctadStore = InMemoryOctadStore<
    ...
    InMemoryVersionStore<verisim_octad::OctadSnapshot>,
    InMemoryProvenanceStore,
    InMemorySpatialStore,
>;

fn create_test_store(vector_dim: usize) -> TestOctadStore {
...
async fn test_create_all_modalities_committed() {
    ...
    let status = &octad.status.modality_status;
    ...
    assert!(retrieved.is_some(), "Created octad must be retrievable");
...
async fn test_create_vector_dimension_mismatch_rolls_back() {
    ...
    assert!(
        all.is_empty(),
        "No octads should exist after failed create — rollback must clean up"
    );
...
async fn test_create_partial_modalities_succeeds() {
    ...
    assert!(!octad.status.modality_status.graph);
    assert!(!octad.status.modality_status.vector);
    assert!(!octad.status.modality_status.tensor);
    assert!(!octad.status.modality_status.semantic);
    assert!(!octad.status.modality_status.provenance);
    assert!(!octad.status.modality_status.spatial);
}
...
async fn test_update_all_modalities_committed() {
    // After a successful update, the octad must reflect the new data and
    ...
    let octad = store.create(create_input).await.unwrap();
...
async fn test_update_nonexistent_fails() {
    ...
    let fake_id = OctadId::new("nonexistent-id");
    let input = OctadBuilder::new()
...
async fn test_update_with_invalid_vector_dimension_rolls_back() {
    ...
    let bad_update = OctadBuilder::new()
        .with_embedding(vec![0.1, 0.2]) // WRONG: 2 dimensions instead of 3
        .build();
...
async fn test_delete_removes_entity() {
    ...
    let octad = store.create(input).await.unwrap();
    assert!(store.get(&octad.id).await.unwrap().is_some());

    store.delete(&octad.id).await.unwrap();
    assert!(
        store.get(&octad.id).await.unwrap().is_none(),
...
async fn test_delete_nonexistent_fails() {
    let store = create_test_store(3);
    let fake_id = OctadId::new("does-not-exist");
...
async fn test_transaction_manager_integrated() {
    ...
    // Create a octad — should begin and commit a transaction
    let input = OctadBuilder::new()
...
async fn test_transaction_modality_versions_increment() {
    ...
    // Update the document
    let update = OctadBuilder::new()
        .with_document("Updated Version Test", "Version 2")
        .build();
    store.update(&octad.id, update).await.unwrap();
...
async fn test_concurrent_creates_succeed() {
    ...
    let all = store.list(100, 0).await.unwrap();
    assert_eq!(all.len(), 10, "All 10 octads should be stored");
}
