...
async fn test_cross_modal_consistency() {
    ...
    // Create a octad with all modalities populated
    ...
    // Verify the octad can be retrieved
    let retrieved = store.get(&octad.id).await.unwrap();
    assert!(retrieved.is_some(), "Octad should be retrievable");
...
async fn test_versioning() {
    ...
    // Update the entity
    let update1 = OctadBuilder::new()
        .with_document("Version 2", "Updated content")
        .build();

    let updated = store.update(&octad.id, update1).await.unwrap();
    assert_eq!(updated.status.version, 2);

    // Update again
    let update2 = OctadBuilder::new()
        .with_document("Version 3", "Final content")
        .build();

    let updated = store.update(&octad.id, update2).await.unwrap();
