...
----
┌─────────────────────────────────────────────────────────────┐
│  Elixir Orchestration Layer (v1.x)                          │
...
│    ├── VeriSim.AdaptiveLearner (feedback loops - v1)        │
│    └── VeriSim.MiniKanren (constraint solving - v3 stub)    │
│              ↓ HTTP                                         │
├─────────────────────────────────────────────────────────────┤
...
- link:WHITEPAPER.md[White Paper] (link:WHITEPAPER.pdf[PDF]) - Architecture rationale and federation design
- link:docs/deployment-modes.adoc[Deployment Modes] - Standalone vs Federated vs Hybrid
- **Deployment Challenges:**
  * link:docs/challenges-standalone.adoc[Standalone Challenges] - Vertical scaling, SPOF, resource interference
  * link:docs/challenges-hybrid.adoc[Hybrid Challenges] - Mixed consistency, cross-boundary drift
  * link:docs/challenges-federated.adoc[Federated Challenges] - Consensus overhead, Byzantine faults, GDPR compliance
...
- **VQL (VeriSim Query Language):**
  * link:docs/vql-grammar.ebnf[VQL Grammar] - Formal EBNF grammar specification (ISO/IEC 14977 compliant)
  * link:docs/vql-formal-semantics.adoc[**VQL Formal Semantics**] - Operational semantics, big-step/small-step evaluation, drift semantics
  * link:docs/vql-type-system.adoc[**VQL Type System**] - Static type rules, dependent types, refinement types, bidirectional type checking
  * link:docs/vql-examples.adoc[VQL Examples] - 42 example queries for all modalities (includes VERSION and DRIFT features)
  * link:docs/vql-architecture.adoc[VQL Architecture] - Dual-path router design (corrected)
  * link:src/vql/VQLParser.res[VQL Parser] - ReScript slipstream parser implementation
  * link:src/vql/VQLExplain.res[VQL EXPLAIN] - Query plan visualization with performance hints
- **Query Optimization & Reversibility:**
  * link:docs/query-optimization-overview.adoc[**Overview**] - Complete guide to query optimization and reversibility
  * link:lib/verisim/query_planner_config.ex[Query Planner Config] - Conservative/Balanced/Aggressive modes with adaptive tuning
  * link:lib/verisim/query_planner_bidirectional.ex[Bidirectional Optimization] - Forward propagation (push predicates) + backward propagation (pull store capabilities)
  * link:docs/reversibility-design.adoc[Reversibility Design] - Time-travel queries, undo operations, drift repair validation (80% free!)
- **Caching:**
  * link:docs/caching-strategy.adoc[Caching Strategy] - Multi-layer cache (L1/L2/L3) with drift-aware invalidation
  * link:docs/cache-sharing-strategy.adoc[Cache Sharing Strategy] - Tiering and sharing across organizations (with diagrams)
  * link:lib/verisim/query_cache.ex[Query Cache] - Implementation with per-modality TTL policies
  * link:lib/verisim/query_router_cached.ex[Cached Query Router] - Wrapper with AST/plan/result caching
- **Error Handling & Safety:**
  * link:docs/error-handling-strategy.adoc[Error Handling Strategy] - Comprehensive error types, recovery strategies, audit trail
  * link:docs/safety-and-fault-tolerance.adoc[Safety & Fault Tolerance] - Memory safety, kernel safety, platform safety, supply chain, socio-technical safety
  * link:docs/safety-theory-applied.adoc[Safety Theory Applied] - Reason, Groeneweg, Rasmussen, Leveson, Hollnagel, Vaughan, Snook applied to VeriSimDB
  * link:src/vql/VQLError.res[VQL Error Types] - Structured error representation (ReScript)
  * link:lib/verisim/error_recovery.ex[Error Recovery] - Retry with backoff, circuit breakers, compensating transactions
  * link:lib/verisim/circuit_breaker.ex[Circuit Breaker] - Prevent cascading failures
- **Backwards Compatibility & Learning:**
  * link:docs/backwards-compatibility.adoc[Backwards Compatibility Strategy] - Semantic versioning, deprecation process, migration strategies, API/VQL/data format evolution
  * link:lib/verisim/adaptive_learner.ex[Adaptive Learner] - Feedback loops for cache TTL, normalization thresholds, drift tolerance, query plan selection (v1)
  * link:docs/minikanren-integration-v3.adoc[**miniKanren Integration (v3 Stub)**] - Constraint-based query optimization, rule synthesis, symbolic reasoning (planned for v3)
- **Drift Handling:**
  * link:docs/drift-handling.adoc[Drift Handling] - Detection, repair strategies, drift-aware queries, federation drift, Byzantine tolerance
  * link:rust-core/verisim-drift/[Drift Detection Crate] - Rust implementation of cross-modal drift detection
- link:docs/security-lessons.lgt[Security Lessons] - Logtalk rules for gitbot-fleet integration
- link:kraft-comparison.adoc[KRaft Comparison] - VeriSimDB vs Kafka KRaft metadata management
