...
# Return the owner/repository pin for an external action or reusable workflow,
# discarding any subpath. Return an empty string for local, malformed, or
# non-repository references.
...
FNR == 1 { wf = FILENAME }
{
  ...
  if (match(line, /^[[:space:]]*-?[[:space:]]*uses:[[:space:]]*(.+)$/, m)) {
    ...
    gsub(/^["']|["']$/, "", raw)
    gsub(/[[:space:]]+$/, "", raw)
    # Self-repository paths (`$/...`) are valid as written. Only an appended
    # ref is invalid: this syntax must not pin a self-repository path.
