...
=== 21. GQL-DT Not Connected to VQL PROOF Runtime (OPEN)
...
**Impact:** PROOF clauses are semantically checked (contract existence, parameter validation) but not formally verified via the Lean dependent type checker. Custom ZKP circuits work (R1CS verification), but the GQL-DT formal path is not wired.
...
**Resolution:** Integrate proven library after GQL-DT (issue 21) is resolved.
