...
=== 24. oxrocksdb-sys C++ Dependency (OPEN — MEDIUM)

**Location:** `rust-core/verisim-graph/` via `oxigraph`

**Current state:** Oxigraph 0.4 unconditionally pulls in `oxrocksdb-sys`, which compiles ~400K lines of C++ from source. This requires `clang-19` and `build-base` in the container image, adding ~1GB to the build stage and ~15 minutes to container build time.

**Impact:** Medium. Container builds are slow. The `clang-19` and RocksDB C++ compilation is the single largest contributor to build time and image size.

**Resolution:** Replace Oxigraph's RocksDB backend with a pure-Rust alternative. Options: `fjall` (LSM-tree, same architecture as RocksDB), `redb` (B-tree, simpler), or move graph storage to Elixir/ETS. The existing `GraphStore` trait abstraction makes this a backend swap.

=== 25. protoc Binary Required at Build Time (OPEN — LOW)

**Location:** `rust-core/verisim-api/build.rs`

**Current state:** `prost-build` shells out to the `protoc` binary to compile `verisim.proto` (179 lines). This requires the `protoc` package in the container.

**Impact:** Low. The proto file is stable and rarely changes.

**Resolution:** Pre-generate the protobuf Rust code, commit it, and remove the build.rs dependency on protoc. Only regenerate when the proto changes.
