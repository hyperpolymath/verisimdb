...
=== 19. VQL Executor Federation Stub — Partially Resolved

**Location:** `elixir-orchestration/lib/verisim/query/vql_executor.ex`

**Partially resolved:** 2026-02-13. `execute_federation_query/7` now delegates to the Rust federation API (`POST /federation/query`) instead of returning empty results. However, the Rust federation endpoint itself returns empty when neither text_query nor vector_query is provided (i.e., bare modality selection over federation returns `[]`).

**Remaining gap:** Bare `SELECT * FROM FEDERATION /universities/*` without a WHERE clause still returns empty results from the Rust side. The Rust federation endpoint needs a "list all" capability.
